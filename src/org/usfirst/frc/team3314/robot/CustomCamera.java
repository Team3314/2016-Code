	...
	*/
	public void centerCamera() {
		...
		if (centerX > 363  && centerX < 383) {
			...
			robot.tankDrive.setStickInputs(-.2, .1875);
		}
		else if (centerX < 363) {
			robot.tankDrive.setStickInputs(.1875, -.2);
		}
	}
	...
	public void centerCameraWithP() {
		...
		if (centerX > 368  && centerX < 378) {
			...
			robot.tankDrive.setStickInputs((-.2 - correction), (.1875 + correction));
		}
		else if (centerX < 368) {
			robot.tankDrive.setStickInputs((.1875 - correction), (-.2 + correction));
		}
	}
	...
	public void centerCameraAngle() {
		...
		if (Math.abs(xAngleError) <= .75){
			...
			robot.tankDrive.setDriveAngle(robot.hal.gyro.angle() + xAngleError);		}
		}
	...
	public void adjustDistance() {
		...
		if (centerY > 175 && centerY < 205) {
			...
			robot.tankDrive.setDriveAngle(robot.hal.gyro.angle());
			robot.tankDrive.setDriveTrainSpeed(-.85);
		}
		else if (centerY < 175) {
			robot.tankDrive.setDriveAngle(robot.hal.gyro.angle());
			robot.tankDrive.setDriveTrainSpeed(.85);
